...
def request_input_from_user(chatbot, history, tip="请输入内容：", var_name="用户输入"):
    """
    向用户发出一个提示并等待输入
    返回：用户的输入字符串
    """
    ...
    chatbot.append([f"📥 {tip}", "等待输入中..."])
    history.append((tip, f"${var_name}$"))  # 这将触发前端输入
    yield from update_ui(chatbot=chatbot, history=history)

    while True:
        # 前端输入内容将覆盖 $变量名$
        for qa in reversed(history):
            if isinstance(qa, tuple) and isinstance(qa[1], str) and not qa[1].startswith("$") and var_name in qa[0]:
                return qa[1]
        import time
        time.sleep(0.5)
