...
def ArgsGeneralWrapper(f):
    ...
    def decorated(request: gradio.Request, cookies: dict, max_length: int, llm_model: str,
                  txt: str, txt2: str, sub_input: str, top_p: float, temperature: float, chatbot: list,
        ...
        # 🛡️ 确保 cookies 不为 None
        if cookies is None:
            print("📛 cookies 为 None，自动修复为空字典")
            cookies = {}

        # 🛡️ 确保关键字段存在，避免 KeyError
        cookies.setdefault("api_key", "")
        cookies.setdefault("most_recent_uploaded", "")
        cookies.setdefault("lock_plugin", None)

        # 📌 注入用户名
        user_name = request.username if request.username is not None else default_user_name
        ...
        if isinstance(plugin_advanced_arg, str):
            ...
            plugin_kwargs = dict(plugin_advanced_arg)  # 拷贝一份
        # ✅ 把 sub_input 加进去
        plugin_kwargs["sub_input"] = sub_input
        ...
        if cookies.get('lock_plugin', None) is None:
            # 正常状态
            if len(args) == 0:  # 插件通道
                result = f(txt_passon, llm_kwargs, plugin_kwargs, chatbot_with_cookie, history, system_prompt, request)
                if inspect.isgenerator(result):
                    yield from result  # 是生成器插件
                else:
                    yield result  # 是普通插件
            else:  # 对话通道，或快捷功能
...
def safe_update_ui(chatbot, history, msg="正常"):
    """
    将 history 中的 $变量名$ 转换为提示字符串，避免 UI 显示占位符
    """
    processed_history = []
    for item in history:
        if isinstance(item, tuple) and isinstance(item[1], str) and item[1].startswith("$") and item[1].endswith("$"):
            var_name = item[1].strip("$")
            processed_history.append((item[0], f"⏳ 等待用户输入 {var_name} ..."))
        else:
            processed_history.append(item)
    yield from update_ui(chatbot=chatbot, history=processed_history, msg=msg)
...
def on_report_generated(cookies:dict, files:List[str], chatbot:ChatBotWithCookies):
    if cookies is not None and "files_to_promote" in cookies:
