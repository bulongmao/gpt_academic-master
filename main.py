...
def main():
    ...
    title_html = f"<h1 align=\"center\">立丰咨询业务垂直领域智能体 1.0</h1>{theme_declaration}"
    ...
    with gr.Blocks(title="立丰咨询业务智能体", theme=set_theme, analytics_enabled=False, css=advanced_css) as app_block:
        gr.HTML(title_html)
        ...
        with gr_L1():
            ...
            with gr_L2(scale=1, elem_id="gpt-panel"):
                ...
                # ✅ 输入组件：常驻显示
                with gr.Row():
                    sub_input = gr.Textbox(label="副输入框", lines=1)
                    dynamic_input_submit = gr.Button("清空", variant="primary")

                dynamic_input_submit.click(
                    fn=lambda: gr.update(value=""),  # 返回空字符串
                    inputs=[],
                    outputs=[sub_input]
                )

                with gr.Row():
                    kb_btn = gr.Button("知识库系统", elem_id="elem_kb_entry", variant="primary")
                kb_btn.click(
                    fn=None,
                    inputs=[],
                    outputs=[],
                    _js="()=>{ window.open('http://localhost:8080', '_blank'); }"
                )
                ...
                with gr.Accordion("业务模块区", open=True, elem_id="plugin-panel") as area_crazy_fn:
                    with gr.Row():
                        gr.Markdown("<small>可读取“输入区”文本/路径作为参数（上传文件自动修正路径）</small>")
                    ...
                    with gr.Row():
                        for index, (k, plugin) in enumerate(plugins.items()):
                            ...
                            plugin['Button'] = plugins[k]['Button'] = gr.Button(k, variant=variant,
                                visible=visible, info_str=f'业务模块区: {info}', elem_id=btn_elem_id).style(size="sm")
                    ...
                    with gr.Row():
                        with gr.Accordion("更多功能", open=False):
                            ...
                            with gr.Row():
                                dropdown = gr.Dropdown(dropdown_fn_list, value=r"点击这里输入「关键词」搜索", label="", show_label=False).style(container=False)
                            with gr.Row():
                                plugin_advanced_arg = gr.Textbox(show_label=True, label="高级参数输入区", visible=False, elem_id="advance_arg_input_legacy",
                                                                 placeholder="这里是特殊功能的高级参数输入区").style(container=False)
                            with gr.Row():
                                switchy_bt = gr.Button(r"请先从列表中选择", variant="secondary", elem_id="elem_switchy_bt").style(size="sm")
                    with gr.Row():
        ...
        # 整理反复出现的控件句柄组合
        input_combo = [cookies, max_length_sl, md_dropdown, txt, txt2, sub_input, top_p, temperature, chatbot, history, system_prompt, plugin_advanced_arg]
        input_combo_order = ["cookies", "max_length_sl", "md_dropdown", "txt", "txt2", "sub_input", "top_p", "temperature", "chatbot", "history", "system_prompt", "plugin_advanced_arg"]
...
print("当前 LLM_MODEL 配置为：", get_conf("LLM_MODEL"))
