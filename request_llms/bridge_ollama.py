...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    while True:
        try:
            ...
            response = requests.post(endpoint, headers=headers, proxies=None,
                                    json=payload, stream=True, timeout=120);break
        except:
            retry += 1
...
def generate_payload(inputs, llm_kwargs, history, system_prompt, stream):
    ...
    # # 自动注入 system prompt，仅对 ollama-deepseek-r1:1.5b 生效
    # if llm_kwargs['llm_model'].strip().lower() in ["ollama-deepseek-r1:", "ollama-deepseek"]:
    #     # 显式禁用 config.py 中的 INIT_SYS_PROMPT 提示词
    #     llm_kwargs['system_prompt'] = ""
    #
    #     system_prompt_auto = {
    #         "role": "system",
    #         "content": """
    # 你是一个结构清晰的大语言模型助手。请严格按以下格式式回答：
    #
    # 【思考过程】
    # 说明你是如何分析问题、理解意图、选择回答策略的。
    #
    # 【正式答复】
    # 给出清晰、简洁、符合要求的回答内容。
    # """
    #     }
    #     messages.insert(0, system_prompt_auto)
