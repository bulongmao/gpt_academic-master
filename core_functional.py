...
def get_core_functions():
    return {
        ...
        "数据趋势分析": {
            ...
            "Prefix":   build_gpt_academic_masked_string_langbased(
                            text_show_chinese=
                                    r"你是一位精通数据分析和趋势洞察的数据分析专家，"
                                    r"请对输入的文本内容进行分析，目标是："
                                    r"找出同类指标（例如某岗位的成本、人力数量等）在不同年份之间的变化趋势，"
                                    r"重点关注保安、保洁、绿化、物业维修、宿管花费成本的年度变化趋势，"
                                    r"请说明每个指标是上升、下降还是波动较大，并解释可能的原因。"
                                    r"最终请给出一个结构清晰的趋势总结，必要时可以用markdown表格辅助展示："
                                ) + "\n\n",
            # [2*] 后缀字符串，会被加在你的输入之后。例如，配合前缀可以把你的输入内容用引号圈起来
            "Suffix":   r"",
            # [3] 按钮颜色 (可选参数，默认 secondary)
        ...
        "Prefix": build_gpt_academic_masked_string_langbased(
                    text_show_chinese=
                        r"你是一位文体转换专家，请将输入文本改写为指定风格的表达方式，"
                        r"可选风格包括：正式风、公文风、科普风、文学风等。"
                        r"请首先识别当前文本风格，然后给出转换后的版本，"
                        r"如需注明风格类型请附在开头或标题中，确保表达自然连贯："
                    )+ "\n\n",
        "Suffix":   r"",      
        "Color":    r"secondary",
        "Visible": True,
        ...
        "Prefix": build_gpt_academic_masked_string_langbased(
                    text_show_chinese=
                        r"你是一位写作批改专家，请对以下输入文本进行语言优化并提供详细反馈。"
                        r"请先输出一版润色后的优化版本，然后用 Markdown 表格列出修改项，"
                        r"包括原句、修改后句子、修改理由，"
                        r"重点关注语法错误、表达不清、逻辑重复等问题。"
                    )+ "\n\n",
        "Suffix":   r"",      
        "Color":    r"secondary",
        "Visible": True,
        ...
        "Prefix": build_gpt_academic_masked_string_langbased(
                    text_show_chinese=
                        r"你是一位信息整理专家，请对以下文本内容提取其核心要点并生成简洁摘要。"
                        r"如果原文为叙述性材料，请以列表形式概括要点；"
                        r"如果原文为报告或说明文，请提炼出主要结论、数据趋势和关键词，"
                        r"摘要长度控制在原文30%左右。"
                    )+ "\n\n",
        "Suffix":   r"",      
        "Color":    r"secondary",
